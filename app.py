...
st.set_page_config(page_title="SQM VENTAGE v5.0.4", layout="wide")
...
st.markdown("""
    ...
    .brand-logo { font-family: 'Inter', sans-serif; font-size: 20px; font-weight: 900; letter-spacing: 2px; color: #ffffff; display: flex; align-items: center; justify-content: center; gap: 10px; }
    .brand-v { background: #ed8936; color: #000; padding: 2px 8px; border-radius: 4px; font-style: italic; }
    .brand-ver { font-size: 10px; color: #ffffff !important; margin-top: 5px; font-weight: 600; opacity: 0.9; }

    [data-testid="stSidebar"] div[data-baseweb="select"], 
    [data-testid="stSidebar"] div[data-baseweb="input"],
    [data-testid="stSidebar"] .stNumberInput div,
    [data-testid="stSidebar"] .stDateInput div { background-color: #1e293b !important; color: white !important; }
    [data-testid="stSidebar"] input { background-color: #1e293b !important; color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; }
    ...
    .main-price-value { color: #ffffff; font-size: 70px; font-weight: 950; line-height: 1; margin: 10px 0; }

    .breakdown-container { display: flex; flex-wrap: wrap; gap: 20px; margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); }
    .breakdown-item { font-size: 14px; color: #94a3b8; }
    .breakdown-item b { color: #ffffff; }
...
user_db = {"admin": "f3e99d9459eeb7ffc4cd407d890fbf1db011208fa12d8edc501a7ec26da106a3"} # Uproszczone dla stabilności kodu

if not st.session_state.authenticated:
    _, col, _ = st.columns([1, 1.2, 1])
    with col:
        st.markdown("<h2 style='text-align:center; color:white; margin-top:50px;'>SQM VENTAGE</h2>", unsafe_allow_html=True)
        u_in = st.text_input("Użytkownik", key="l_u")
...
# --- SIDEBAR ---
with st.sidebar:
    st.markdown('<div class="brand-container"><div class="brand-logo"><span class="brand-v">V</span> SQM VENTAGE</div><div class="brand-ver">SYSTEM LOGISTYCZNY VER. 5.0.4</div></div>', unsafe_allow_html=True)
...
if results:
    ...
    with cl:
        # Tutał był błąd - teraz cały HTML jest w jednym f-stringu i jednej funkcji
        import_html = f'<div class="breakdown-item">Import: <b>€ {best["imp"]:,.0f}</b></div>' if trip_type != "TYLKO DOSTAWA (ONE-WAY)" else ""
        ...
        st.markdown(f"""
            <div class="hero-card">
                ...
                <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:15px;margin-top:25px;'>
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>TRANZYT</div><div style='color:white;font-size:18px;font-weight:900;'>{best['transit']} dni</div></div>
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>TARGI</div><div style='color:white;font-size:18px;font-weight:900;'>{days_stay} dni</div></div>
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>AUTO</div><div style='color:white;font-size:18px;font-weight:900;'>{best['Pojazd']}</div></div>
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>LOAD</div><div style='color:white;font-size:18px;font-weight:900;'>{best['load']:.0f}%</div></div>
                </div>
            </div>
        """, unsafe_allow_html=True)
        
        st.write("### 🚛 PORÓWNANIE POJAZDÓW")
        for r in sorted(results, key=lambda x: x['Total']):
            is_best = "alt-best" if r['Pojazd'] == best['Pojazd'] else ""
            st.markdown(f'<div class="alt-card {is_best}"><div><b>{r["Pojazd"]}</b> <small>({r["Szt"]} szt. | {r["load"]:.0f}%)</small></div><div class="price-tag">€ {r["Total"]:,.2f}</div></div>', unsafe_allow_html=True)
    ...
    with cr:
        ...
        st.info(f"**Data montażu:** {d_start}")
