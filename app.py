...
st.set_page_config(page_title="SQM LOGISTICS v16.7", layout="wide")

# --- CSS Z POPRAWKĄ CZYTELNOŚCI ANALIZY ---
st.markdown("""
    <style>
    ...
    .alt-card b { 
        color: #ffffff !important; /* Biały dla typu pojazdu */
        font-size: 18px;
        font-weight: 800;
        text-transform: uppercase;
    }
    .alt-card small { 
        color: #94a3b8 !important; /* Szary dla liczby sztuk */
        font-size: 13px;
        margin-left: 8px;
    }
    .alt-best { 
...
if not st.session_state.authenticated and not st.session_state.logout_triggered:
    ...
    if c_token and c_token in user_db:
        st.session_state.authenticated = True; st.session_state.user_id = c_token
...
with st.sidebar:
    ...
    st.markdown(f"<div style='background:#1e293b;padding:15px;border-radius:10px;border-left:4px solid #ed8936;margin:20px 0;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>OPERATOR</div><div style='color:white;font-size:14px;font-weight:800;'>{st.session_state.user_id.upper()}</div></div>", unsafe_allow_html=True)
...
if not df_baza.empty:
    for v_type, cap in caps.items():
        ...
        if not res.empty:
            ...
            park = (days_stay * cfg.get('PARKING_DAY', 30) * v_count); stay = r['Postoj'] * days_stay * v_count
            results.append({"Pojazd": v_type, "Szt": v_count, "Total": exp+imp+stay+park+ata+ferry, "transit": transit_days, "load": min(100, (w_eff/(v_count*cap))*100)})
...
if results:
    ...
    with cl:
        st.markdown(f"""<div class="hero-card"><div style='color:#ed8936;font-size:14px;font-weight:800;'>SUGEROWANA STAWKA (NETTO)</div><div class="main-price-value">€ {best['Total']:,.2f}</div>
                    ...
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>NAJTAŃSZY</div><div style='color:white;font-size:22px;font-weight:900;'>{best['Pojazd']}</div></div>
        ...
        for r in sorted(results, key=lambda x: x['Total']):
            ...
            st.markdown(f"""
                <div class="alt-card {is_best}">
                    <div><b>{r['Pojazd']}</b> <small>({r['Szt']} szt. | Załadunek {r['load']:.0f}%)</small></div>
