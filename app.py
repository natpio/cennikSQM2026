...
with tab_calc:
    if final_results:
        ...
        with c_left:
            costs_html = "".join([f"<div class='cost-item'>{k}<b>€ {v:,.2f}</b></div>" for k, v in best['brk'].items() if v > 0])
            st.markdown(f"""
                <div class="hero-card">
                    <div style="color:#ed8936; font-size:13px; font-weight:800; letter-spacing:1px;">NAJTAŃSZA OPCJA LOGISTYCZNA (NETTO)</div>
