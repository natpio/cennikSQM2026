...
with st.sidebar:
    st.markdown("""
        <div class="brand-container">
            ...
            <div class="brand-ver">SYSTEM LOGISTYCZNY VER. 5.0.4</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    if trip_type == "PEŁNA TRASA (EXP+IMP)":
        ...
        st.caption("Tryb One-Way: brak powrotu i postoju")
...
if results:
    ...
    with cl:
        # Konstrukcja bloku HTML dla głównej karty
        ...
        st.markdown(f"""
            <div class="hero-card">
                ...
                <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:15px;margin-top:25px;'>
                    ...
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>TARGI</div><div style='color:white;font-size:22px;font-weight:900;'>{days_stay}d</div></div>
                    ...
                    <div style='background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;'><div style='color:#94a3b8;font-size:10px;font-weight:700;'>ŁADUNEK</div><div style='color:white;font-size:22px;font-weight:900;'>{best['load']:.0f}%</div></div>
                </div>
            </div>
        """, unsafe_allow_html=True)
        
        st.write("### 🚛 PORÓWNANIE KOSZTÓW FLOTY")
        for r in sorted(results, key=lambda x: x['Total']):
            is_best = "alt-best" if r['Pojazd'] == best['Pojazd'] else ""
            st.markdown(f'<div class="alt-card {is_best}"><div><b>{r["Pojazd"]}</b> <small>({r["Szt"]} szt. | Załadowanie {r["load"]:.0f}%)</small></div><div class="price-tag">€ {r["Total"]:,.2f}</div></div>', unsafe_allow_html=True)
